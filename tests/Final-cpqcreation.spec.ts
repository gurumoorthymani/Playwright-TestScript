 ...
 await frame.getByRole('button', { name: 'Add Products' }).click();
  ...
  for (let i = 0; i < frameCount; i++) {
    ...
    if (isVisible) {
      ...
      await frame.locator('#firstColumn >> role=combobox')
      ...
      await frame.locator('#secondColumn >> role=combobox')
...
await saveFrame.getByRole('button', { name: 'Save', exact: true }).waitFor({ timeout: 15000 });
